...
class Institution:
    ...
    # ── 어디로 가고 어디에 거는가 (기획서 §6.4 ③단계) ──
    # **창구 안내가 전화번호보다 먼저다.** 전화를 걸면 무엇을 물어야 할지 또 판단해야
    # 하지만, 창구에서는 한 문장만 말하면 된다. 이 사용자층에게는 갈 곳이 더 실행 가능하다.
    # 갈 곳이 하나로 정해지는 항목(신분증·통장·주민등록·증명서)일 때만 채운다.
    desk_place: str = ""  # "가까운 주민센터"
    desk_say: str = ""  # "주민등록 재등록하러 왔어요"
    # 연락처는 hotlines.HOTLINES의 키로 적는다. 번호를 여기 직접 쓰면 두 곳이 어긋난다.
    contact_key: str = ""
