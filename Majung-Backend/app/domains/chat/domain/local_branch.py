...
def _line(inst: SupportInstitution) -> str:
    phone = f" {inst.phone}" if inst.phone else ""
    return f"{inst.name}{phone} · {inst.address}"


def answer_for(sido: str, sigungu: str, found: list[SupportInstitution]) -> LocalBranchAnswer:
    """찾은 공단 기관을 답변에 넣을 형태로.

    **가까운 곳을 먼저 낸다.** 시군구가 맞는 것이 있으면 그것만 내고, 없으면 같은
    시도의 것을 낸다. 공단 기관은 전국에 서른여덟 곳뿐이라 시군구로 거르면 대개
    비고, 그때 "없다"고 답하면 처음 문제로 돌아간다.
    """
    if not found:
        return LocalBranchAnswer(injection="")

    same_town = [i for i in found if sigungu and sigungu in i.address]
    picked = same_town or found

    lines = "\n".join(f"- {_line(i)}" for i in picked[:_MAX_LINES])
