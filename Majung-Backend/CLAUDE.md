...
│   ├── centers/                 # 기관 안내 — 좌표를 받아 가까운 순으로 낸다 (F-1)
...
7. **저장하는 것은 암호화하고, 로그에는 남기지 않는다** — 본선은 가입 정보·죄목·대화 내역을 저장한다(기획서 §9). 다만 **로그·에러 리포트에 사용자 입력 원문을 남기지 않는다.** 저장과 로깅은 다른 문제다
   ...
   - **위치 좌표를 받고 저장한다** (2026-08-26 결정 F-1로 뒤집힘). `app_user.place_enc`에 암호화해 **덮어쓴다** — 마지막 한 자리만 남기며, 쌓으면 동선 기록이 된다
   - 좌표는 대한민국 범위 밖이면 받지 않고(위도 33~39·경도 124~132), **조회 조건을 로그에 남기지 않는다**
