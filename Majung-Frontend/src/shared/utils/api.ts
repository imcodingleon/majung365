 ...
 */
const SER_BASE = process.env.EXPO_PUBLIC_SER_URL ?? "http://localhost:8500";
/**
 * 🔒 터널 접근용 '약한 게이트' 토큰(선택). EXPO_PUBLIC_이라 번들에 노출됨 → 진짜 비밀 아님.
 * 실제 방어: CORS 오리진 제한 + 테스트할 때만 터널 on + 서버 오디오 무영속. 데모 한정 사용.
 */
const SER_TOKEN = process.env.EXPO_PUBLIC_SER_TOKEN ?? "";
 ...
 */
export async function postVoiceAnalyze(
  audio: Blob,
  ...
  const res = await fetch(`${SER_BASE}/analyze`, {
    method: "POST",
    headers: SER_TOKEN ? { "X-Majung-Token": SER_TOKEN } : undefined,
    body: form,
  });
