...
- 위치: `expo-location` — 행정동 판정은 `src/shared/location/`이 경계 데이터를 안고 기기 안에서 한다. **좌표는 서버로 보내고 서버가 저장한다** (2026-08-26 결정 F-1로 뒤집힘). 구글 지도로 가는 것도 같은 날 허용했다
...
2. **서비스명 표기 "마중365"**
3. **저리터러시 UI**: 큰 글씨·카드형 선택지·한 화면에 한 가지
   - **화면 문구의 어체는 `../_bmad-output/specs/spec-majung-2nd/copy-voice.md`가 정한다** (2026-08-31 결정). 디자이너 시안의 합니다체 고지투를 채택했고, 그전까지 쓰던 해요체 대화투를 뒤집었다. **화면에 나갈 말을 쓰기 전에 그 문서를 읽는다**
   - `humanize-korean` 스킬은 AI 티를 걷어내는 용도로만 쓴다. 어체 판정은 그 문서가 한다
4. **화면에 쓰지 않는 말**: "죄목" · "영역"("분야")
   - **"수용 사유"는 써도 된다** (2026-08-31 결정). "어떤 일로 계셨나요"를 시안 문구로 바꿨다
...
8. **위치 좌표를 서버로 보낸다** (2026-08-26 결정 F-1로 뒤집힘). `getCenters`가 `lat`·`lng`를 함께 보내고, 위치가 정해질 때 `patchMe`가 서버에 남긴다
   - **좌표는 짝으로만 다룬다.** 하나만 있으면 없는 것으로 친다 — 반쪽 좌표로 거리를 재면 엉뚱한 곳이 나온다
   - 지역을 직접 고른 사용자에게는 좌표가 없다. 그 길이 막히면 안 된다 (§5.4)
   - 기기의 `localStorage`에도 좌표까지 남긴다. 새로고침 뒤에도 그 자리를 기준으로 재야 한다
...
- **창 너비로 화면 구조를 가르지 않는다.** 웹은 `AppFrame`이 모든 화면을 440px 프레임에 묶는데, `useWindowDimensions`와 tailwind `lg:`는 **브라우저 창**을 본다. PC에서 열면 판정만 켜지고 정작 넣을 자리는 없다 — 실제로 지도가 48px 세로 띠로 눌렸다
...
| 데스크톱 셸 | **모바일 단일 프레임 하나뿐이다.** 지도 화면을 넓은 폭에서 2열로 보내려고 `useIsDesktop`을 되살렸다가 2026-08-26에 다시 걷어냈다 — 웹은 `AppFrame`이 모든 화면을 440px에 묶으므로 PC에서 판정만 켜지고 자리는 없었다 |
| 음성 감정 인식 | **범위에서 제외** |
